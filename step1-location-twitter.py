#######################################################################################
# step1-location-twitter.py                                                           #
#                                                                                     #
# Purpose:  Twitter Mining Workshop. Gets tweets by location (MO)                     #
# Python:  2.7 and up                                                                 #
...
class MyStreamer(TwythonStreamer):
    def __init__(self, out_file, maximum, *args):
        ...
        self._count = 0
        #maximum stores the maximum number of tweets be downloaded
...
with open("twitter_download-location.json", 'w') as out_file:
    stream = MyStreamer(out_file, 100, API_KEY, API_SECRET,
                        OAUTH_TOKEN, OAUTH_TOKEN_SECRET)
    print "file stored:", stream.out_file
    stream.statuses.filter(language='en', locations = box_mo)
